...
export default class Utils {
	...
	*/
	...
	public static getIpsInRange(start:string, end:string):string[] {
		let res = [];
		let chunks = start.split(".");
		let startBin = 0;
		let endBin = 0;
		for (let i = 0; i < chunks.length; i++) {
			startBin |= (parseInt(chunks[i]) & 0xff) << (24-8*i);
		}
		chunks = end.split(".");
		for (let i = 0; i < chunks.length; i++) {
			endBin |= (parseInt(chunks[i]) & 0xff) << (24-8*i);
		}
		if(endBin - startBin > 1000) {
			return null;
		}
	...
	public static decodeFromURI(str:string):any {
